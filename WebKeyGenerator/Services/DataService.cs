...
namespace WebKeyGenerator.Services
{
    public class DataService : IDataService
    {
        ...
        public void CreateRequest(DoctorRequest req, IConfiguration config)
        {
            ...
            try
            {
                ...
                Email.Send(config, $"Ваша заявка принята.\r\n" +
                    $"Ваша учетная запись будет активирована после проверки администратором.",req.Email);
            }
            catch (Exception ex)
            {
        ...
        {
            var doc = db.Doctors.Include(e => e.User).Include(e=>e.Schedulle).FirstOrDefault(e => e.User.Id == id);
